...
class DomoApplication:
    ...
    async def get_all_schedules(self, full_auth: DomoFullAuth = None):
        ...
        self.jobs_schedule = schedules.sort_values(['hour', 'minute'], ascending = True).reset_index(drop = True)
    ...
    async def find_next_job_schedule(self) -> dmdj.DomoTrigger_Schedule:
        ...
        schedules_grouped = self.jobs_schedule.groupby(['hour', 'minute']).size().reset_index(name = 'cnt_schedule')
        
        # print(schedules_grouped)
        # print(df_all)

        schedules_interpolated = pd.merge(df_all, schedules_grouped, how = 'left', on = ['hour', 'minute'])
        
        schedules_interpolated['cnt_schedule'] = schedules_interpolated['cnt_schedule'].fillna(value = 0)
        schedules_interpolated.sort_values(['cnt_schedule', 'hour', 'minute'], ascending = True, inplace = True)
