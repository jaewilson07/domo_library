...
def generate_body_create_group(group_name: str,
    ...
    body = {"name": group_name, "type": group_type or 'open', "description": description or ''}
...
async def create_group(full_auth: DomoFullAuth,
    ...
    body = generate_body_create_group(group_name=group_name, group_type=group_type, description=description)
...
def generate_body_update_group_membership(group_id: str,
    ...
    if add_owner_user_arr:
        body.update({"addOwners": [{"type": "USER", "id": str(userId)} for userId in add_owner_user_arr]})

    if remove_owner_user_arr:
        body.update({"removeOwners": [{"type": "USER", "id": str(userId)} for userId in remove_owner_user_arr]})

    if remove_user_arr:
        body.update({"removeMembers": [{"type": "USER", "id": str(userId)} for userId in remove_user_arr]})
    if add_user_arr:
        body.update({"addMembers": [{"type": "USER", "id": str(userId)} for userId in add_user_arr]})
