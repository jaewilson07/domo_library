...
class SupportTicket(Base):
    ...
    def _create_from_domo_obj(cls, json_obj:dict, status_labels_list = None):
        ...
        ticket = cls(
            ...
            date_closed = tu.trello_timestr_to_datetime(dd.ClosedDate) if dd.ClosedDate else None,
            last_updated = tu.trello_timestr_to_datetime(dd_last_updated) if dd_last_updated else None,
            status = dd.Status,
    ...
    def map_domo_property_to_label(self, property_name, labels_list = None):
        if getattr(self ,property_name):
            property_match = next((label for label in labels_list if label.name ==getattr(self , property_name)), None)
    ...
    def map_trellocard_labels(self, labels_list):
        ...
        for card_label in self.labels:
            label_match = next((label for label in labels_list if label.id ==card_label.id), None)
