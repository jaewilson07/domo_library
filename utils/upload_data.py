...
async def upload_data(instance_auth,
    ...
    try:
        ...
        if debug_prn:
            print(f"starting {instance_auth.domo_instance} - {data_fn.__name__}")
...
async def upload_data_with_date(instance_auth,
    ...
    print(f"'🎬 upload_with_data: starting retrieval {start_date}, {end_date}, {instance_auth.domo_instance}")
    ...
    if debug_prn:
        print(f'🧻 upload_with_data: starting upload {len(upload_df)} rows for {instance_auth.domo_instance}')
    ...
    if debug_prn:
        print(f'🎉 upload_with_data : finished uploading {len(upload_df)} rows for {instance_auth.domo_instance}')
    return res