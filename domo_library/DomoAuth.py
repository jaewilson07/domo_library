...
class _DomoAuth_Required:
    ...
    async def is_token(
        self,
        token_name: str = None,  # give the token a friendly name (ex. developer, full_auth)
    ) -> bool:
        ...
        print(
            f"{token_str if token_name else ''}token retrieved from {self.domo_instance}"
        )
        return True
...
class InvalidCredentialsError(DomoErrror):
    ...
    def __init__(
        self, status: int = None, message="invalid credentials", domo_instance=None
    ):
...
class InvalidInstanceError(DomoErrror):
    ...
    def __init__(
        self, status: int = None, message="invalid instance", domo_instance: str = None
    ):
        super().__init__(status=status, message=message, domo_instance=domo_instance)
