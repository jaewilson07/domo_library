...
# %% auto 0
__all__ = ['DatasetSchema_AuthNotProvidedError', 'DomoDataset_Schema_Column', 'DomoDataset_Schema',
           'DatasetTags_AuthNotProvidedError', 'DatasetTags_SetTagsError', 'DomoDataset_Tags', 'DomoDataset']
...
class DomoDataset_Schema:
    ...
    async def get(
        ...
        if self.dataset and (not self.dataset.auth and not auth):
            raise DatasetSchema_AuthNotProvidedError(self.dataset.id)
...
class DatasetTags_SetTagsError(Exception):
    ...
    def __init__(self, id, domo_instance):
        message = f"failed to set tags on dataset - {id} in {domo_instance}"
        super().__init__(message)
    ...
    async def set(
        ...
        if res.status != 200:
            raise DatasetTags_SetTagsError(
                dataset_id=dataset_id, domo_instance=domo_instance
            )
