...
# %% auto 0
__all__ = ['AccountRoute_ErrorTypes', 'AccountRoute_Error', 'get_accounts', 'GetAccount_NoMatch', 'GetAccount_NoConfigRetrieved',
...
@dataclass 
class AccountRoute_ErrorTypes(Enum):
    """Enumeration of Account Route error types and messages. Can be manipulated as a dataframe"""
    CreateAccount_Error: str = "Unable to create account"
    UpdateAccount_Error: str = "Unable to update account"
    ShareAccount_Error: str = "Unable to share account"
    ShareAccount_Error_AlreadyShared: str = "Unable to share previously shared account"

class AccountRoute_Error(de.DomoError):
    """Account Route Error handling"""
    def __init__(
        self, 
        status, 
        response,
        ...
        error_type=AccountRoute_ErrorTypes,
    ):
        message = f"{error_type.name} {account_id} - {response} { (' - ' + info) or ''}"
...
async def update_account_config(
    ...
    if res.status == 400 and res.response == "Bad Request":
        raise AccountRoute_Error(
            error_type=AccountRoute_ErrorTypes.UpdateAccount_Error,
            status=res.status,
...
async def create_account(
    ...
    if not res.is_success:
        raise AccountRoute_Error(
            error_type=AccountRoute_ErrorTypes.CreateAccount_Error,
            status=res.status,
...
async def share_account_v2(
    ...
    if res.status == 500 and res.response == "Internal Server Error":
        raise AccountRoute_Error(
            error_type=AccountRoute_ErrorTypes.ShareAccount_Error_AlreadyShared,
            account_id=account_id,
            status=res.status,
    ...
    if not res.status == 200:
        raise AccountRoute_Error(
            error_type=AccountRoute_ErrorTypes.ShareAccount_Error,
            account_id=account_id,
            status=res.status,
            response=res.response,
            domo_instance=auth.domo_instance,
