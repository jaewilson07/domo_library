...
async def _from_aiohttp_response(
    cls: ResponseGetData, res: aiohttp.ClientResponse  # requests response object
) -> ResponseGetData:
    ...
    if res.ok and "application/json" in res.headers.get("Content-Type", {}):
        try:
            return cls(status=res.status, response=await res.json(), is_success=True)
        ...
        # handle if unable to decode json()
            ...
            return cls(status=res.status, response=await res.read(), is_success=True)

        # response is text
    elif res.ok:
        return cls(status=res.status, response=await res.text(), is_success=True)
