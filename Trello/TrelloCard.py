...
class UpdateCard:
    ...
    async def name(self, new_value, auth, debug: bool = False):
        await card_routes.put_card_attribute(
    ...
    async def add_label(self, new_value, auth, debug: bool = False):
        await card_routes.post_card_attribute(
            auth=auth or self._auth,
            card_id=self._card.id,
            attribute="idLabels",
            value=new_value,
    ...
    async def remove_label(self, new_value, auth, debug: bool = False):
        await card_routes.delete_card_attribute(
            auth=auth or self._auth,
            card_id=self._card.id,
            attribute="idLabels",
            value=new_value,
...
class TrelloCard(Base):
    ...
    async def get_props(self, card_id: str = None, debug: bool = False):
        ...
        res = await card_routes.get_card_by_id(
            auth=self.auth, card_id=card_id, debug=debug
        )
    ...
    async def _create_card(
        ...
        res = await card_routes.post_new_card(
            auth=auth, card_properties=card_properties, debug=debug, session=session
        )
    ...
    async def _upsert_card(
        ...
        res = await card_routes.get_search_cards_by_name(
            auth, search_name=name, allow_partial_match=False, debug=debug
        )
        ...
        if res.status == 200:
            ...
            match_card = (
                next((card for card in search_cards if card.get("name") == name), None)
    ...
    async def delete_card(
        self, debug: bool = False, session: aiohttp.ClientSession = None
    ):
        return await card_routes.delete_card(
            auth=self.auth, card_id=self.id, debug=debug, session=session
        )
