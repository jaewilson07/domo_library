...
async def get_data(
    ...
    if debug:
        print(f"\n🐞 debugging auth 🐞")
        pprint({'auth': auth, 'url': url, 'params': query_params, 'headers': headers, 'data': data})
    ...
    try:
        if isinstance(auth, ta.TrelloWebAuth):
            ...
            print("🐞 using oauth 🐞")
            session = session or aiohttp.ClientSession(request_class=aioauthlib.OAuthRequest, timeout = timeout)
            ...
            print('invalid creds')
            raise ex.AuthRequiredException(message = f"invalid Trello Auth object - {type(auth)}")  
        ...
        if response.status != 200:
            ...
            raise ex.ResourceUnavailable("%s at %s" % (response.text, url), response)
        ...
        print("🐞 timeout error running backup 🐞")
