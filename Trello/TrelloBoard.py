...
class GetBoardCards:
    ...
    async def _process_board_card(
        ...
        res = await board_routes.get_board_cards(
            auth=auth, board_id=self._board.id, card_filter=card_filter, debug=debug
        )
...
class GetBoardLists:
    ...
    async def _process_board_list(
        ...
        self.lists = []
        res = await board_routes.get_board_lists(
            auth=auth, board_id=self._board.id, list_filter=list_filter, debug=debug
        )
    ...
    async def closed(self, debug: bool = False, log_results: bool = False):
        return await self._process_board_list(list_filter="closed", debug=debug)
...
class TrelloLabel:
    ...
    def _create_from_json(cls, json_obj):
        ...
        return cls(id=dd.id, name=dd.name)
...
class TrelloBoard(Base):
    ...
    async def create_label(self, label_color, label_name, auth=None):
        ...
        res = await board_routes.create_label(
            label_color, label_name, auth=auth or self.auth, board_id=self.id
        )
        return res

    async def get_labels(self, auth=None, debug: bool = False):
        res = await board_routes.get_board_labels(
            auth=auth or self.auth, board_id=self.id, debug=debug
        )
    ...
    async def get_props(
        self, board_id: str = None, auth: ta.TrelloAuth = None, debug: bool = False
    ):
        ...
        res = await board_routes.get_board_by_id(
            auth=auth, board_id=board_id, debug=debug
        )
