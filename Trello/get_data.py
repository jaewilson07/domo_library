...
async def get_data(
    ...
    try:
        if isinstance(auth, ta.TrelloWebAuth):
            ...
            client = aioauthlib.AsyncOAuth1Client(
                session=session,
            ...
            response = await getattr(client, http_method.lower())(
                url=url, headers=headers, data=data, params=query_params
            )
        else:
            print("invalid creds")
        ...
        return ResponseGetData(status=-1, response="error", is_success=False, auth=auth)
